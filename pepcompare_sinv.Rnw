...
SV_seq=readAAStringSet(file.path('inputdata','prot_sequences.txt'))
...
<<single_mapped_peps>>=
ProtIDs=mapPeptides(PeptideSet=input_raw$sequence, 
...
name_comp=paste('diff',paste(sample_combi[,i],
                             collapse='_'),sep='_')
diff_table=data.frame(as.character(proteins$ENSGid),
                      as.character(proteins$Know_RBP),
                      fit$coefficients,
                      fit$p.value,fit$p.adj)
...
for (i in 1:dim(sample_combi)[2]){
  ...
  fit$p.adj=p.adjust(fit$p.value,method="BH")
  fit$p.adj[is.na(fit$p.adj)]=1
  volplot=cbind(fit$coefficient,fit$p.adj)
  colnames(volplot)=c('coefficients','p.adj')
...
for (i in 1:dim(sample_combi)[2]){
  for (j in 1:2){
    ...
    for (s in Sources) {
      GSEAnew = GSEA
      GSEAnew$T = GSEAnew$T[GSEAnew$T$source == s,]
      pdf(width=10,height=7,file=file.path(GSEAdir,sprintf("GSEA-%s.pdf",s)))
      x = plotGSEA(GSEAnew, main = s, maxp = 0.05,nrOfSets = 0, cex=0.6)
      dev.off()
  h = x / 20 + 0.25
